        ...
        try:
            async with self.get_client() as client:
                await client.put_object(
                    Body=file, Bucket=self.bucket, Key=s3_file, ContentType=content_type
                )
            return (
                f"Upload Success to {s3_file}",
                s3_file,
            )  # TODO - поменять имя, Возвращает сообщение об успешной загрузке и путь к файлу
    ...
    async def upload_files_to_s3(self, file_group: list[tuple[bytes, str, str]]):
        ...
        tasks = [
            self.upload_file_to_s3(
                file_data, s3_file, content_type
            )  # Создаем задачу для загрузки каждого файла
