...
class UserDialogsDAO(BaseDAOmongo):
    ...
    async def get_user_dialogs(cls, user_id, last_message_datetime=None, limit=15):
        ...
        pipeline = [
            ...
            {
                "$unwind": "$activeDialogs"
            },  # Раскрываем каждый элемент массива activeDialogs в отдельный документ
            {
                "$sort": {"activeDialogs.lastMessage.sendTime": -1}
            },  # Сортируем диалоги по дате последнего сообщения
        ]

        if last_message_datetime:
            pipeline.append(
                {"$match": {"activeDialogs.lastMessage.sendTime": {"$lte": last_message_datetime}}}
            )

        pipeline.append(
            {"$replaceRoot": {"newRoot": "$activeDialogs"}}
        )  # Заменяем корень документа на activeDialogs
