...
) -> SUsersMyInfo:  # испраить - другая схема - различные варианты
...
async def update_info(
    ...
    if files:
        uploaded_files_paths = await S3Client(settings.PROFILE_BUCKET).upload_on_storage(
            user_id, files
        )
        if uploaded_files_paths:
            photo = uploaded_files_paths[
                0
            ]  # функция возвращает нам список всегда, поэтому вытаскиваем первый ссылочный элемент
            # process_pic.delay(uploaded_files_paths[0]) # task TODO Реализовать систему изменения размерности с сохранением в S3

    variables = {"name": name, "text": text, "photo": photo}
    ...
    if photo and user.photo:
        delete_old_pic.delay(
            user.photo, settings.PROFILE_BUCKET
        )  # запустили таску на удаление старого фото из s3
...
async def update_location(
    latitude: str | None, longitude: str | None, user: Users = Depends(get_current_user)
):
